...
def _get_client_ip(request: Request) -> str:
    # X-Real-IP is set by nginx to $remote_addr (the actual connecting IP) and
    # cannot be spoofed by clients. X-Forwarded-For is NOT used here because
    # nginx appends to any client-supplied value, so the first entry is
    # attacker-controlled and would allow rate-limit bypass.

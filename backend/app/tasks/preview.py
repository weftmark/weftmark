...
_PROJECT_SVG_CELL_PX = 8  # matches landing page step-tracking view default
...
@celery_app.task(
    ...
    soft_time_limit=120,
    time_limit=150,
    name="app.tasks.preview.generate_project_drawdown_svg",
)
def generate_project_drawdown_svg(self: Task, project_id: str) -> None:
    asyncio.run(_generate_project_svg(self, uuid.UUID(project_id)))
        ...
        async with async_session() as db:
            ...
            try:
                ...
                def _render() -> str:
                    ...
                    if color_replacements:
                        rendering.apply_color_replacements(d, color_replacements)
                    return rendering.render_drawdown_svg(d, _PROJECT_SVG_CELL_PX)

                svg_text = await asyncio.to_thread(_render)
                old_path = project.drawdown_svg_path
                project.drawdown_svg_path = storage.save_project_drawdown_svg(svg_text)
                await db.commit()
                if old_path:
                    try:
                        storage._delete(old_path)
...
@celery_app.task(
    ...
    soft_time_limit=300,
    time_limit=360,
    name="app.tasks.preview.backfill_all_project_drawdown_svgs",
)
def backfill_all_project_drawdown_svgs(self: Task) -> dict:
    """Dispatch generate_project_drawdown_svg for every project missing a stored SVG."""
    return asyncio.run(_backfill_all_project_svgs())


async def _backfill_all_project_svgs() -> dict:
    from sqlalchemy import select
        ...
        async with async_session() as db:
            projects = (
                await db.scalars(
                    ...
                    .where(
                        Project.deleted_at.is_(None),
                        Project.drawdown_svg_path.is_(None),
                    )
                    .order_by(Project.created_at)
                )
            ).all()
    ...
    result = {"dispatched": dispatched, "skipped": skipped}
    log.info("backfill_all_project_drawdown_svgs_complete %s", result)
    return result
